...
class Generator {
    ...
    public function generateTransaction($method, AbstractTransaction $transaction, $username, $password, $language=null) {
        ...
        $root = $this->document->createElementNS('http://gateway.sepaexpress.shop/Schema/V2/Transaction', 'transaction');
    ...
    public function generateOptions($username, $password, $identifier, $parameters=array()) {
        $this->document = new \DOMDocument('1.0', 'utf-8');
        $this->document->formatOutput = true;
        $root = $this->document->createElementNS('http://gateway.sepaexpress.shop/Schema/V2/Options', 'options');
        $this->document->appendChild($root);
